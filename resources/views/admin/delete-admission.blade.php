...
<!-- <button
       ...
       <li class="nav-item">
       <a class="nav-link " href="{{route('admission-approved')}}">
          Edit Approvel
       </a>
        </li>
    
        <li class="nav-item">
       <a class="nav-link active" href="{{route('delete-admission-approved')}}">
         Delete Approvel
       </a>
        </li>
               </ul>
  ...
  <div class="row" id="showsingle">
   ...
   var table = $('#datatable').DataTable({
    ...
    bFilter: false,
    ajax: {
        url: "{{ route('approved-delete-admission') }}",
        // type: 'GET',
        // data: function (d) {
        //    ;
        // }
        ...
        },
        {
            data: 'image',
                ...
                render: function (data, type, full, meta) {
                    return type === 'display' ?
                        '<img  style="height:100px;" src="' + "{{ asset('events') }}/" + data + '" alt="Image" class="img-thumbnail">' :
                        data;
                }
},
        {
...
$('body').on('click', '.delete', function(e) {
    ...
    $.ajax({
        url : "{{route('admission-data-delete')}}",
        type : "GET",
        data : {id : id},
 ...
 $('body').on('click', '.reject', function(e) {
    ...
    // alert(id);
    $.ajax({
        url : "{{route('admission-delete-reject')}}",
        type : "GET",
        data : {id : id},
        success : function(response){
            if (response.status == "success") {
