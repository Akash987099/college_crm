...
<!-- <button
    ...
    <!-- <button  type="button" class="btn btn-primary mb-3 " id="university_search">Search</button> -->
      <ul class="nav nav-pills flex-column flex-md-row mb-3">
       ...
       <li class="nav-item">
       <a class="nav-link active" href="{{route('facility-approved')}}">
          Edit Approvel
       </a>
        </li>
    
        <li class="nav-item">
       <a class="nav-link" href="{{route('delete-facility-approved')}}">
         Delete Approvel
       </a>
        </li>
  ...
  <div class="row" id="showsingle">
   ...
   var table = $('#datatable').DataTable({
    ...
    bFilter: false,
    ajax: {
        url: "{{ route('approved-facility-view') }}",
        // type: 'GET',
        // data: function (d) {
        //    ;
        // }
...
$('body').on('click', '.viewall', function(e) {
    ...
    $.ajax({
        url : "{{route('get-facility-data-byid')}}",
        type : "GET",
        data : {id : id},
 ...
 $('body').on('click' , '.tglbtn' , function(){
    ...
    //   alert(id);
				$.ajax({
					url: "{{ route('change-facility-status') }}",
					dataType : "json",
					type: "get",
					data : {id:id},
					success : function(response) {
 ...
 $('#updata').on('click', function(e) {
    ...
    $.ajax({
        url: "{{ route('approved-facility-update') }}",
        dataType: "json",
        type: "POST",
        data: formData,
        processData: false,
