...
<!-- <button
       ...
       <li class="nav-item">
       <a class="nav-link active" href="{{route('admission-approved')}}">
          Edit Approvel
       </a>
        </li>
    
        <li class="nav-item">
       <a class="nav-link" href="{{route('delete-admission-approved')}}">
         Delete Approvel
       </a>
        </li>
               </ul>
  ...
  <div class="row" id="showsingle">
   ...
   var table = $('#datatable').DataTable({
    ...
    bFilter: false,
    ajax: {
        url: "{{ route('admission-approved-view') }}",
        // type: 'GET',
        // data: function (d) {
        //    ;
        // }
...
$('body').on('click', '.viewall', function(e) {
    ...
    $.ajax({
        url : "{{route('get-admission-data-byid')}}",
        type : "GET",
        data : {id : id},
 ...
 $('body').on('click' , '.tglbtn' , function(){
    ...
    //   alert(id);
				$.ajax({
					url: "{{ route('change-admission-status') }}",
					dataType : "json",
					type: "get",
					data : {id:id},
					success : function(response) {
 ...
 $('#updata').on('click', function(e) {
    ...
    $.ajax({
        url: "{{ route('approved-admission-update') }}",
        dataType: "json",
        type: "POST",
        data: formData,
        processData: false,
