    ...
    @csrf
      ...
      <div class="row" id="description-container">
        ...
        <div class="mb-3 col-md-6">
          ...
          <input class="form-control specialchar" type="file" name="images[]" id="images" multiple />
        </div>
      </div>
      </div>
    ...
    @csrf
     <input type="text" name="userupdateid" id="user_id_view" hidden>
      <div class="row" id="description-container">
        ...
        <div class="mb-3 col-md-6">
          <label for="firstName" class="form-label ">Image <span class="required text-err required">*</span></label>
                            <input
                              class="form-control specialchar"
                              type="file"
                              name="image"
                ...
                if (response.status == "success") {
                    ...
                    $.each(response.message, function (i, message) {
                        if (i == "editorContent") {
                            ...
                            $('#images').after('<span id="images_error" class="err" style="color:red">' + message + '</span>');
                        }
                    });
                }
            },
        ...
        return result.trim(); 
...
},
            {
                ...
                render: function (data, type, full, meta) {
                    return type === 'display' ?
                        '<img  style="height:70px;" src="' + "{{ asset('placement') }}/" + data + '" alt="Image" class="img-thumbnail">' :
                        data;
                }
            },
            {
        ...
        success: function(response) {
            if (response) {
                $.each(response, function(index, item){
                    ...
                    $('#updatelink').val(item.link);
                    
                    CKEDITOR.instances.programeditor1.setData(item.content);

                    if (item.image) {
                        var imgElement = $('<img>').attr('src', "{{ asset('placement/') }}/" + item.image);
          ...
          if (response.status == "success") { 
                ...
                $.each(response.message, function (i, message) {
                    if (i == "link") {
                        ...
                        $('#upcomingupdate').after('<span id="editorContent_error" class="err" style="color:red">' + message + '</span>');
                    }
                });
            }
        }
