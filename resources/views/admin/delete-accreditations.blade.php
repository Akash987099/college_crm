...
<!-- <button
       ...
       <li class="nav-item">
       <a class="nav-link " href="{{route('accreditations-approved')}}">
          Edit Approvel
       </a>
        </li>
    
        <li class="nav-item">
       <a class="nav-link active" href="{{route('delete-accreditations')}}">
         Delete Approvel
       </a>
        </li>
               </ul>
                  ...
                  <div class="table-responsive text-nowrap">
                    ...
                    <thead class="">
                      ...
                      <th class="fs-6 text-capitalize">S.No.</th>
									...
									<th class="fs-6 text-capitalize"> Image</th>
									<th>Action</th>
                      </tr>
                    </thead>
  ...
  <div class="row" id="showsingle">
   ...
   var table = $('#datatable').DataTable({
    ...
    bFilter: false,
    ajax: {
        url: "{{ route('approved-delete-accreditations') }}",
        // type: 'GET',
        // data: function (d) {
        //    ;
        // }
        ...
        },
        {
            data: 'image',
                ...
                render: function (data, type, full, meta) {
                    return type === 'display' ?
                        '<img  style="height:100px;" src="' + "{{ asset('accreditations_master') }}/" + data + '" alt="Image" class="img-thumbnail">' :
                        data;
                }
},
        {
...
$('body').on('click', '.delete', function(e) {
    ...
    $.ajax({
        url : "{{route('accreditations-data-delete')}}",
        type : "GET",
        data : {id : id},
 ...
 $('body').on('click', '.reject', function(e) {
    ...
    // alert(id);
    $.ajax({
        url : "{{route('accreditations-delete-reject')}}",
        type : "GET",
        data : {id : id},
        success : function(response){
            if (response.status == "success") {
